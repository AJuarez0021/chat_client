...
void AddText(HWND hWnd,UINT,string strText);
	     ...
	     case ID_OK:
			 {
			  ...
			  if(!strIP){
	        	MessageBox(hWnd,TEXT("Error no hay memoria suficiente"),TEXT("Cliente"),MB_ICONERROR);
		        EndDialog(hWnd,1);
			  }
			  ...
			  if(bResultPort && uResultIP && uResultName){
			   ...
			   MessageBox(hWnd,"Error: Necesita introducir el nombre, puerto y el host","Cliente",MB_ICONERROR);
			 }
			  break;
		 case ID_CANCEL:
  ...
  case WM_COMMAND:
	   switch(LOWORD(wParam)){
	     case ID_CONECTAR:		
			  ...
			  if(bConnect){               			
				 if(s.Connect()){
			      ...
			      SetText(hWnd,IDC_STATIC_STATUS,"Cliente Conectado");		
			      ...
			      SetText(hWnd,IDC_STATIC_STATUS,"No se pudo conectar el cliente");	
		 ...
		 case ID_DESCONECTAR:
              ...
              EnableWindow(GetDlgItem(hWnd,ID_DESCONECTAR),FALSE);
			  EnableWindow(GetDlgItem(hWnd,ID_SAVE),TRUE);
			  SetText(hWnd,IDC_STATIC_STATUS,"Cliente Desconectado");
			  bDisconnect=true;
			  break;
		 case ID_CLEAR:
		 ...
		 case ID_SEND:
			 {
			  ...
			  if(!strText){
			   MessageBox(hWnd,"Error no hay memoria suficiente","Cliente",MB_ICONERROR);
		       EndDialog(hWnd,1);
			  }
